...
exports.index = async function (req, res) {
  try {
    const restaurants = await Restaurant.findAll(
      {
        attributes: ['id', 'name', 'description', 'address', 'postalCode', 'url', 'shippingCosts', 'averageServiceMinutes', 'email', 'phone', 'logo', 'ordenar', 'heroImage', 'status', 'restaurantCategoryId'],
        include:
        {
...
exports.show = async function (req, res) {
  try {
    const restaurantToOrder = await Restaurant.findByPk(req.params.restaurantId)
    let restaurant
    if (restaurantToOrder.ordenar === true) {
      restaurant = await Restaurant.findByPk(req.params.restaurantId, {
        ...
        order: [[{ model: Product, as: 'products' }, 'price', 'ASC']]
      })
    } else {
      restaurant = await Restaurant.findByPk(req.params.restaurantId, {
        attributes: { exclude: ['userId'] },
...
exports.ordenacion = async function (req, res) {
  try {
    const restaurantToChange = await Restaurant.findOne({ where: { id: req.params.restaurantId } })
    const nuevaOrdenacion = !(restaurantToChange.ordenar)
    await Restaurant.update({ ordenar: nuevaOrdenacion }, { where: { id: req.params.restaurantId } })
    const updatedRestaurant = await Restaurant.findByPk(req.params.restaurantId)
    res.json(updatedRestaurant)
  } catch (err) {
    res.status(500).send(err)
